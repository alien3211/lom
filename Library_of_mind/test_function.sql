...
BEGIN
    ...
    SET librery_id := ( SELECT id_lib FROM keys_list WHERE key_name = key_n);
    ...
    INSERT INTO tmp_lib VALUES(v_id);
    select * from tmp_lib NATURAL JOIN library_list;
    DELETE FROM tmp_lib;
END
//
...
SELECT p.type AS CHILDREN, s.type AS PARENT FROM types_list p JOIN types_list s ON p.id_parent = s.id_type;
//
